...
\subsection{互模拟关系与观察等价性}\label{ch:bisimulation}
   ...
   \end{definition}
   \subsubsection{$\mathbb{VPC}_\mathsf{Th}$的符号互模拟}\label{ch:vpc_symbolic_bisimulation}
...
\begin{itemize}
   \item[(1)] $t$上的所有节点$N$，$N\in [A]_{\varphi \mathcal{E}}$。
   \item[(2)] {
...
\begin{definition}[条件等价森林]
   ...
   $A$关于$\varphi\mathcal{E}$的条件等价森林有且仅有$\varphi_i\mathcal{E}$的等价树或等价森林,$i\in I$。
\end{definition}
...
\begin{proof}
   ...
   给定$A_0\mathcal{E}_1A_1\mathcal{E}_2A_2\dots A_{k-1}\mathcal{E}_k A_k$，
   由于$\mathcal{E}=(\bigcup_{i\in I}\mathcal{E}_i)^*$，
   我们可以得到$A_1\mathcal{E}A_k$，
   若$\mathcal{E}$是一个符号互模拟关系，
   则对于$A_0\rightsquigarrow_{\varphi\mathcal{E}}\stackrel{\lambda}{\rightarrow}_{\varphi} \mathcal{C}\in \mathcal{T}_{\mathbb{RVPC}_{\mathsf{Th}}},\mathcal{C}\neq [A_0]_{\varphi\mathcal{E}}$，
   存在$\varphi$的划分$\{\varphi_i\}_{i\in I}$和集合
   $\{A_k\rightsquigarrow_{\varphi_i\mathcal{E}}\stackrel{\lambda}{\rightarrow}_{\psi_i}\mathcal{C}|\mathsf{Th}\vdash \varphi_i\Rightarrow\psi_i\}$可模拟该操作
   (对于条件$q$-转移的证明方式和条件$l$-转移本质上是相同的，在后续的证明中仅证明条件$l$-转移)。
   ...
   和集合$\{A_1\rightsquigarrow_{\varphi_i\mathcal{E}}\stackrel{\lambda}{\rightarrow}_{\psi_i}\mathcal{C}\}$
   来模拟$A_0$的动作。
   ...
   我们可以递归的构建出$A_1$的条件等价森林。

   \begin{itemize}
      \item {
         \textbf{$t_{A_0}$的根节点$A_0$只有一个儿子节点$A_0'$。}
         若$A_0'\in[A_0]_{\varphi\mathcal{E}_{i_1}}$，
         我们将根据$A_0'$的$\varphi\mathcal{E}_{i_1}$树构建$A_1$的条件等价森林。
      ...
      \item {
         ...
         $A_0$到$A_0^j$的边被标记为$(\psi, p_j)$。

         若所有$A_0^j\in [A_0]_{\varphi\mathcal{E}_{i_1}}$，
         我们将根据$A_0^1$构建$A_1$的条件等价森林。
         ...
         我们根据$A_0^1$构建每个$N'$的等价森林。
      }
      \item {
         \textbf{$t_{A_0}$的根节点$A_0$可以作$A_0\stackrel{l}{\rightarrow}_{\varphi} L'$。}
         根据定义存在$\varphi$的划分$\{\varphi_i\}_{i\in I}$和集合
         $\{A_1\rightsquigarrow_{\varphi_i\mathcal{E}_{i_1}}\stackrel{l}{\rightarrow}_{\varphi_i} [L']_{\varphi_i\mathcal{E}_{i_1}}\}$。
      }
   \end{itemize}

   通过上述递归的构建，我们可以得出图中的森林:

\end{proof}

现在我们可以定义$\mathbb{RVPC}_{\mathsf{Th}}$上的观察等价性了。
\begin{definition}
...
证明条件操作运算的封闭性等同于证明对于任意$S,T\in \mathcal{T}_{\mathbb{RVPC}_{\mathsf{Th}}}$，
$S=_{\mathbb{RVPC}_{\mathsf{Th}}} T$可以推出
$\varphi S =_{\mathbb{RVPC}_{\mathsf{Th}}} \varphi T$。
为方便证明，后文中$=_{\mathbb{RVPC}_{\mathsf{Th}}}$简记作$=$，绝对等价用$\equiv$表示。

令二元关系$\mathcal{R}\equiv\{(\varphi S,\varphi T)|S=T\}$，
二元关系$\mathcal{R}'\equiv(R\cup =)^*$。
我们证明$\mathcal{R}'$是一个符号互模拟关系。
