...
这个神域帮组我们进行条件的判定，函数的计算和提供变量的取值范围。
...
Fu在The Value-Passing Calculus中取缔了神域的概念，
提出了一个封闭的传值进程模型。
...
\subsection{互模拟关系与观察等价性}\label{ch:bisimulation}
   ...
   在提出CCS时Milner提出了\textit{观察等价(observation equivalence)}与\textit{弱互模拟(weak bisimulation)}\cite{2}。
   Glabbeek和Weijland提出的\textit{分支互模拟(branching bisimulation)}\cite{6}也是十分著名的研究。

   对于概率模型的等价性，目前有对同步概率进程模型互模拟关系的研究\cite{13}，
   仅适用于有限状态的概率进程模型的研究\cite{14,15}等。
...
\begin{example}\label{eg:conditional}
   ...
   \begin{figure}[!htbp]
      \small
      \centering
      \includegraphics[width=5cm]{../figure/example_condition2.png}
      \caption[]{}
